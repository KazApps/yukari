...
impl Thread {
    ...
    pub fn search(&mut self, depth: i32, mut alpha: i32, beta: i32, ply: usize, tt: &[TtEntry], excluded_move: Option<Move>) -> i32 {
        ...
        let (sigma, s) = match (depth, piece_count) {
            (1, 0) => (38, 0),
            (1, 1) => (71, 0),
            (1, 2) => (50, 0),
            (1, 3) => (31, 0),
        ...
        if excluded_move.is_none() && !self.board[ply].in_check() && alpha >= -1000 && beta <= 1000 && !expected_pvnode && depth <= 5 {
            let bound = beta + sigma;
        ...
        if excluded_move.is_none() && !self.board[ply].in_check() && alpha >= -1000 && beta <= 1000 && !expected_pvnode && depth == 1 {
            let bound = alpha - sigma;
